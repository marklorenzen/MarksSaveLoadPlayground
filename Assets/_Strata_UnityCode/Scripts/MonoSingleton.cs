...
//
// I've split the namespace to make clear that the following class interoperates with UnityEngine.
// Some day we may want to assert that code not requiring UnityEngine is thread safe.
//
            ...
            if (System.Threading.Monitor.TryEnter(g_lock)) // We might be allocating from within GetInstance()
            {
                try
                {
                    if (null == g_instance)
                    {
                        ...
                        {
                            Debug.LogWarningFormat("[Singleton] Instance '{0}' moving this singleton object to the scene root so it can persist. ({1})\n"
